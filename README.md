...
1. EC2 인스턴스 프로파일 설정하기
2. 신뢰 정책 파일 만들기 => json
3. IAM 역할 생성  최대 세션 시간 7200 초, 이름은 마음대로 
4. API 서버에 필요한 최소 권한 정책 json 파일 만들고,

   s3에저장하고 가져오고 삭제권한 , s3버킷의 listBucket 권한 (새로생성해도 되고, 있는거 사용해도된다)

   클라우드 와치의 로그그룹, 로그스트림, 풋이벤트 권한 (클라우드와치의 로그그룹은 어제만든것)

6. 위의 json 으로 정책생성하고 (이름은 마음대로)
7. 역할에 정책 연결해본다.
8. 인스턴트 프로파일 생성하기
9. 역하을 인스턴스 프로파일에 추가 
